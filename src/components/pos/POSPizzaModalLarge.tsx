 ...
 * Comfortable spacing, gaps between sections, larger text.
...
const btnSmall = "h-7 px-2.5 text-xs rounded border font-medium transition-colors mx-px my-px text-foreground inline-flex items-center justify-center text-center leading-tight whitespace-nowrap min-w-0";
const labelBox = "h-7 px-2 text-xs font-medium rounded grid place-items-center text-center leading-tight whitespace-normal min-w-0";
...
export const POSPizzaModalLarge = (props: POSPizzaModalProps) => {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent
        className="max-w-[1020px] w-[95vw] p-2.5 pt-2 overflow-hidden max-h-[96vh] text-slate-900 !duration-300 !animate-in !fade-in-0 !zoom-in-100 data-[state=closed]:!zoom-out-100 !gap-0"
      ...
      >
        ...
        <div className="flex flex-wrap items-center gap-1.5 pb-0.5 border-b border-slate-200 pr-12 flex-shrink-0">
          <span className="font-serif text-xs font-bold px-4 py-1.5 rounded whitespace-nowrap uppercase" style={{ ...blueStyle, minWidth: 120 }}>{item.name}</span>
          {item.sizes?.map(size => (
        ...
        <div className="flex flex-col gap-0.5 flex-1 min-h-0 overflow-hidden" style={{ flex: '1 1 0%' }}>
          {/* ROW 2: Cheese */}
                ...
                {m.availableExtraToppings.map(topping => {
                  ...
                  return (
                    <div key={topping.id} className="flex items-center gap-1.5">
                      <button onClick={() => m.toggleExtraTopping(topping)} className="flex items-center justify-start px-2 py-1.5 rounded border font-medium truncate" style={{ ...(isSelected ? blueStyle : darkStyle), flex: '1 1 0%', minWidth: 0 }}>
                      ...
                      {m.isLargePizza ? (
                        SIDE_OPTIONS.map(side => {
                          ...
                          return (
                            ...
                            }} className="py-1.5 px-2 text-[11px] rounded border font-medium text-center whitespace-nowrap" style={isThisSideActive ? blueStyle : darkStyle}>
                              {side.label}
                            </button>
                          );
                        })
                      ) : (
                        <button type="button" onClick={() => m.toggleExtraTopping(topping)} className="py-1.5 px-3 text-[11px] rounded border font-medium text-center whitespace-nowrap" style={isSelected ? blueStyle : darkStyle}>
                          Whole
                        </button>
                      )}
                    </div>
        ...
        {/* FOOTER */}
        <div className="flex items-center gap-1.5 flex-shrink-0" style={{ borderTop: '1px solid #9ab8c8', paddingTop: 4, marginTop: 2 }}>
          <input type="text" value={m.note} onChange={(e) => m.handleNoteChange(e.target.value)} placeholder={m.shortcutPlaceholder} className="flex-[2] px-2.5 py-2 text-xs border border-slate-800 rounded bg-white text-slate-800 placeholder:text-slate-400" />
          <input type="text" inputMode="decimal" value={m.extraAmount || ''} onChange={(e) => m.handleExtraAmountChange(e.target.value)} placeholder="0.00" className="w-[90px] px-2 py-2 text-xs border border-slate-800 rounded bg-white text-center text-slate-800" />
          <span className="text-lg font-bold text-slate-900 whitespace-nowrap min-w-[70px] text-center">${m.totalPrice.toFixed(2)}</span>
          <Button variant="outline" onClick={onClose} className="text-sm px-4 py-2 h-auto font-semibold" style={{ backgroundColor: '#f4a27a', borderColor: '#f4a27a', color: '#1a1a1a' }}>Cancel</Button>
          <Button variant="default" onClick={m.handleAddToOrder} disabled={!m.selectedSize || !m.selectedCrust} className="text-sm px-6 py-2 h-auto font-bold bg-slate-900 text-white hover:bg-slate-800">

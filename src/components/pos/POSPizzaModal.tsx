    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-[95vw] w-[1400px] max-h-[90vh] p-3 gap-0 overflow-hidden">
        ...
        <div className="flex items-center gap-4 pb-2 border-b">
          <h2 className="font-serif text-sm font-bold whitespace-nowrap" style={{ color: '#1a8ccc' }}>
            {item.name}
          </h2>
          ...
          {availableCrusts.length > 0 && (
            <div className="flex items-center gap-2">
              ...
              {availableCrusts.map((crust) => (
                ...
                >
                  {crust.name}
                  {crust.name.toLowerCase().includes('gluten') && ` +$${GLUTEN_FREE_PRICE}`}
                </button>
              ))}
            </div>
          )}
          ...
          <div className="ml-auto flex items-center gap-3">
            <span className="text-lg font-bold" style={{ color: '#1a8ccc' }}>
              ${totalPrice.toFixed(2)}
            </span>
            <Button variant="outline" onClick={onClose} className="h-7 px-3 text-xs">
              Cancel
            </Button>
            <Button
          ...
          <div className="flex items-center gap-1">
            ...
            {['None', 'Mozz', 'Dairy Free'].map((cheese, idx) => {
              ...
              return (
                ...
                >
                  {cheese}
                  {val === 'Dairy Free' && ` +$${selectedSize?.name === 'Small 10"' ? 2 : 3}`}
                </button>
              );
            })}
          ...
          <div className="flex items-center gap-1">
            ...
            <span
              className={cn(
                ...
                (leftSpicy === 'medium' || rightSpicy === 'medium') && 'text-[#1a8ccc]'
              )}
            >
              Med
            </span>
                ...
                {(['left', 'whole', 'right'] as Side[]).map((side) => {
                  ...
                  const isActiveM =
                    side === 'whole'
                      ? isWholeM
                      : side === 'left'
                        ? leftSpicy === 'medium' && !isWholeM
                        : rightSpicy === 'medium' && !isWholeM;
                  const hotWhole = leftSpicy === 'hot' && rightSpicy === 'hot';
                  const isDisabledM =
                    hotWhole ||
                    (side === 'left' && leftSpicy === 'hot') ||
                    (side === 'right' && rightSpicy === 'hot') ||
                    (side === 'whole' && (leftSpicy === 'hot' || rightSpicy === 'hot'));
                  return (
                    <SpicySideBtn
                      key={side}
                      side={side}
            ...
            <span
              className={cn(
                'text-[10px] font-medium px-1',
                (leftSpicy === 'hot' || rightSpicy === 'hot') && 'text-[#1a8ccc]'
              )}
            >
              Hot
            </span>
                ...
                {(['left', 'whole', 'right'] as Side[]).map((side) => {
                  ...
                  const isActiveH =
                    side === 'whole'
                      ? isWholeH
                      : side === 'left'
                        ? leftSpicy === 'hot' && !isWholeH
                        : rightSpicy === 'hot' && !isWholeH;
                  const medWhole = leftSpicy === 'medium' && rightSpicy === 'medium';
                  const isDisabledH =
                    medWhole ||
                    (side === 'left' && leftSpicy === 'medium') ||
                    (side === 'right' && rightSpicy === 'medium') ||
                    (side === 'whole' && (leftSpicy === 'medium' || rightSpicy === 'medium'));
                  return (
                    <SpicySideBtn
                      key={side}
                      side={side}
                        ...
                        {(['left', 'whole', 'right'] as Side[]).map((side) => (
                          <button
                            ...
                            className={cn(
                              ...
                              sel?.side === side ? btnActive : btnInactive
                            )}
                          >
                            {side === 'left' ? 'L' : side === 'whole' ? 'W' : 'R'}
                          </button>
        ...
        <div className="flex items-center gap-1 py-1.5 border-b flex-wrap">
          ...
          <button
            ...
            className={cn(btn, !selectedSauceId ? btnActive : btnInactive)}
          >
            None
          </button>
          {availableSauces.map((sauce) => (
            <button
              ...
              className={cn(btn, selectedSauceId === sauce.id ? btnActive : btnInactive)}
            >
              {sauce.name}
            </button>
          ))}
          ...
          {(['less', 'normal', 'extra'] as const).map((qty) => (
            <button
              ...
              className={cn(
                btn,
                !selectedSauceId ? 'opacity-40' : sauceQuantity === qty ? btnActive : btnInactive
              )}
            >
              {qty === 'less' ? 'Less' : qty === 'normal' ? 'Reg' : 'Xtra'}
            </button>
                ...
                {defaultToppings.map((topping) => {
                  ...
                  return (
                    <div
                      key={topping.id}
                      className={cn(
                        'rounded p-1 border text-center',
                        isRemoved ? 'border-red-300 bg-red-50' : 'border-gray-200'
                      )}
                    >
                      <button
                        ...
                        <div className="flex gap-0.5 mt-1 justify-center">
                          {(['less', 'regular', 'extra'] as ToppingQuantity[]).map((q) => (
                            <button
                              ...
                              className={cn(
                                'px-1 py-0.5 text-[9px] rounded border',
                                topping.quantity === q ? btnActive : btnInactive
                              )}
                            >
                            ...
                            <>
                              {(['left', 'whole', 'right'] as PizzaSide[]).map((s) => (
                                <button
                                  ...
                                  className={cn(
                                    'px-1 py-0.5 text-[9px] rounded border',
                                    topping.side === s ? btnActive : btnInactive
                                  )}
                                >
                  ...
                  return (
                    <div
                      ...
                      className={cn(
                        ...
                        isSel ? 'border-[#1a8ccc] bg-[#1a8ccc]/10' : 'border-gray-200'
                      )}
                    >
                      <button
                        ...
                        <div className="flex gap-0.5 flex-shrink-0">
                          {(['left', 'whole', 'right'] as PizzaSide[]).map((s) => (
                            <button
                              ...
                              className={cn(
                                'px-1 py-0.5 text-[9px] rounded border',
                                sel?.side === s ? btnActive : btnInactive
                              )}
                            >
                              {s === 'left' ? 'L' : s === 'whole' ? 'W' : 'R'}
                            </button>
